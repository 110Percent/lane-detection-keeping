...
for line in lanes:
    ...
    for point1, point2 in zip(x_list, y_list):
        ...
        print(x1, y1)
        cv2.line(np.int32(img), (x1, y1), (x2, y2), (255, 0, 0), thickness = 10)
...
cv2.polylines(img, [src_pts.astype(np.int32)],True, (255,0,0), thickness=5)
...
for line in lanes:
    ...
    for point in points:
        ...
        # t_point[0][0][1] = abs(t_point[0][0][1]  - ORIGINAL_SIZE[1])
...
plt.clf()
for line in lines:
    cv2.polylines(warped, [np.array(line).astype(np.int32)], False, (255,0,0), thickness=5)
plt.imshow(warped)
