...
\begin{acronym}
	...
	\acro{FRX}{Functional Requirement X}

	\acro{KNC}{(Lane) Keeping and Control}

	\acro{ML}{Machine Learning}

	\acro{MVP}{Minimum Viable Product}

	\acro{NFRX}{Non-Functional Requirement X}

	\acro{PID}{Proportional-Integral-Derivative}

	\acro{ROS}{Robot Operating System}

	\acro{SAE}{Society for Automotive Engineers}
...
\begin{quote}
	``We've arranged a society on science and technology in which nobody understands anything about science and technology,
	and this combustible mixture of ignorance and power sooner or later is going to blow up in our faces. I mean, who is
	running the science and technology in a democracy if the people don't know anything about it?''
...
\textbf{Preamble: Source Code}

The source code for the project and this report can be found at:
\url{https://github.com/110Percent/lane-detection-keeping}

The repository contains everything needed to run the project, including source code, Dockerfiles and instructions /
...
\begin{table}[]
	...
	\end{tabular}
	\caption{Top 10 TuSimple Lane Detection Benchmark models, March 2024 (Source: PapersWithCode \cite{PWPTuSimple}).}
...
Overall, the project would be best described by the group as a success.

For starters, the team was able to accomplish several of the original design goals for the project. The team was able to create a system that was capable of following lanes in a road. Furthermore, in more simple cases the system that was designed could follow roads for a long time before veering offroad or crashing into any obstacles. Simply achieving this state of the project has been considered a success by the group.

Furthermore, the group was also able to set a very strong foundation for future groups in the form of the frameworks set up by the team. The code created for running CARLA as well as the network interfacing between a remote laptop and the JetAcker robot will allow future groups that are more skilled in the field of control systems to much more easily implement their new ideas and try new things out. By that metric, the group finds that we were able to success in the  project objective of making our code easy for future groups to work on.

However, there were a few shortfalls with the project. Notably, the team was unable to fulfill the desired control related functional requirements. Despite their best efforts, the team was not able to achieve the maximum lateral error of 0.45 meters which was an important part of the project due to making sure the vehicle does not veer off to the side of the road.

\subsection{Changes from Proposal}

By the end of the project, we had a lot of changes from our proposal.

For starters, we ended up not making our own CNN and instead had to opt for a pre-existing model to use. This was a major shock to our team as we had expected that we would be designing and making our own. However, because we used the pre-existing one that had a very high performance it ended up being a better decision in the end.

Furthermore, we had several design changes that took place in the K\&C section of the proposal that were subsequently changed by the final product. For starters, we originally planned to start out with a PID Controller and then end up working on an MPC model by the end of the project, given sufficient time. However, we found that the PID controller was not as straightforward as we had originally anticipated and as a result, had to look for alternative methods to complete the control system. We ended up with the Stanley controller, which proved to be an efficient control system for our work.

We also ran into several changes with respect to the HiWonder device we were using. At the start of the project, it was expected that our group would be working with another project group that had the JetAuto robot and the two groups would be sharing. However, partway through the term we were able to acquire a JetAcker robot which allowed us to work with our own equipment that was more suited to the work we were doing.
...
\begin{figure}[H]
	...
	\begin{minipage}{.45\textwidth}
		...
		\caption{Lateral error over time for merge path trial 2}
	...
	\begin{minipage}{.45\textwidth}
		...
		\caption{Top-down view of vehicle path compared to lane center for merge path Trial 2}
...
\begin{figure}[H]
	...
	\begin{minipage}{.45\textwidth}
		...
		\caption{Lateral error over time for merge path trial 3}
	...
	\begin{minipage}{.45\textwidth}
		...
		\caption{Top-down view of vehicle path compared to lane center for merge path Trial 3}
...
\begin{figure}[H]
	...
	\begin{minipage}{.45\textwidth}
		...
		\caption{Lateral error over time for merge path trial 4}
	...
	\begin{minipage}{.45\textwidth}
		...
		\caption{Top-down view of vehicle path compared to lane center for merge path Trial 4}
...
\begin{figure}[H]
	...
	\begin{minipage}{.45\textwidth}
		...
		\caption{Lateral error over time for merge path trial 5}
	...
	\begin{minipage}{.45\textwidth}
		...
		\caption{Top-down view of vehicle path compared to lane center for merge path Trial 5}
...
\includepdf[pages=-,pagecommand={},width=\textwidth]{external/proposal.pdf}
